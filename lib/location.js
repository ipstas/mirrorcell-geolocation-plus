...
function filter (pos) {
  ...
  if (Location._options.distanceFilter.enabled) {
    log("filter", "Filtering distance");
    var d = getDistance(old, pos);
    log("filter", "Filter: " + Location._options.distanceFilter.range + ". Actual Distance - " + d);
  ...
  if (Location._options.timeFilter.enabled) {
    ...
    log("filter", "Filter - Has Been " + Location._options.timeFilter.span + " Seconds? " + tf);
    if (!tf) {
      return null;
    }
  }
  ...
  if (Location._options.accuracyFilter.enabled && pos.coords.accuracy && !(isNaN(pos.coords.accuracy))) {
    log("filter", "Accuracy" + pos.coords.accuracy);
    if (pos.coords.accuracy > Location._options.accuracyFilter.rating) {
      log("filter", "Accuracy filter: Not accurate enough");
      return null;
    }
  }
...
function log (caller, message, meta) {
  message = message || "";
  caller = caller || "";
  message = "[Location2." + caller + "] " + message;
  meta = meta || "";
  Location.debug && console.log(message, meta);
}
function error (err) {
...
Location = {
  ...
  getLastPosition:       function () {
    if (options.persistent) {
      ...
      log("getLastPosition", "Location Error: You've set persistent storage to false");
    }
  },
  locate:                function (callback, failureCallback) {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(function (pos) {
        log("locate", "Get Current Position Received New Position: " + JSON.stringify(pos));
  ...
  startWatching:         function (callback, failureCallback) {
    log("startWatching", "Initiated", { _watching: this._watching, options: this._watchOptions });

    const onSuccess = function (pos) {
      log("startWatching", "Received New Position: " + JSON.stringify(pos));
      var filtered = filter(pos);
      if (filtered) {
    ...
    const onFailure = function (error) {
      log("startWatching", "Failed", { error });
      failureCallback && failureCallback();
    };

    if (!this._watching && navigator.geolocation) {
      this._watchId = navigator.geolocation.watchPosition(onSuccess, onFailure, this._watchOptions);
      this._watching = true;
    }
  },
  stopWatching:          function () {
    log("stopWatching", "Initiated", { watching: this._watching, watchId: this._watchId });
  ...
  setWatchOptions:       function (options) {
    if (!options) {
      log("setWatchOptions", "You must provide an options object");
    } else {
  ...
  setGetPositionOptions: function (options) {
    if (!options) {
      log("setGetPositionOptions", "You must provide an options object");
    } else {
...
function getDistance (p1, p2) {
  if (p1 && p2) {
    log("getDistance", "Getting distance for", { p1, p2 });
...
function isSecondsAway (date, seconds) {
  var now = new Date();
  log("isSecondsAway", "Time Calc: " + (now.getTime() - date.getTime()));
  log("isSecondsAway", seconds + " Seconds: " + (seconds * 1000) + ' In Milliseconds');
