...
function storePosition (pos) {
  ...
  options.persistent && localStorage && localStorage.setItem('flybuy:lastPosition', string);
...
Location = {
  ...
  locate:                function (callback, failureCallback) {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(function (pos) {
        this.log("locate", "Get Current Position Received New Position: " + JSON.stringify(pos));
  ...
  startWatching:         function (callback, failureCallback) {
    ...
    ctx.log("startWatching", "Initiated",
      { watching: this._watching, options: this._watchOptions });
    ...
    const onFailure = function (error) {
      ctx.log("startWatching", "Failed", { error });
...
function getDistance (p1, p2) {
  if (p1 && p2) {
    Location.log("getDistance", "Getting distance for", { p1, p2 });
